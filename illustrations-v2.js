...
const MarketingIllustrations = (() => {
  const industries = {
    ...
    steel: {
      name: '製鋼業',
      events: ['ロール交換治具の配置で、段取り時間は変わる', '表面疵で、圧延ラインが止まった', '田中さんが不在。原料配合に合う出湯温度は？', 'ロール摩耗基準の変更を、標準作業書へ', '別の圧延ラインにも、改善事例を伝えたい'],
      descriptions: ['離れたロール交換治具をまとめる配置を考える圧延担当者', '表面探傷検査でロール疵のある圧延材を確認する作業者', 'スクラップの銅混入率と出湯温度・脱酸剤量を前に迷う担当者', 'ロール摩耗基準の変更を図で標準書に書き込む手元', '圧延ラインの担当者どうしが改善図を見て相談する場面'],
      object: 'ロール交換治具配置',
      success: '治具配置を変え、段取り時間を短縮',
      failure: '手順を見誤り、板厚公差外れが発生',
      repeat: '新人の佐藤も、同じ治具配置に気づいた'
    },
    nonferrous: {
      name: '非鉄金属製造業',
      events: ['陽極酸化槽の治具配置で、段取り時間は変わる', '膜厚むらで、表面処理ラインが止まった', '田中さんが不在。合金組成に合う鋳造温度は？', '治具配置基準の変更を、標準作業書へ', '別の表面処理槽にも、改善事例を伝えたい'],
      descriptions: ['槽内で離れた治具をまとめる配置を考える表面処理担当者', '膜厚計測で陽極酸化皮膜のむらを確認する検査担当者', '合金組成のケイ素比率差と鋳造温度・湯流れ速度を前に迷う担当者', '治具配置基準の変更を図で標準書に書き込む手元', '表面処理ラインの担当者どうしが改善図を見て相談する場面'],
      object: '陽極酸化槽治具配置',
      success: '治具配置を変え、段取り時間を短縮',
      failure: '手順を見誤り、膜厚むらが発生',
      repeat: '新人の佐藤も、同じ治具配置に気づいた'
    },
    chemical: {
      name: '化学工業',
      events: ['原料投入順序で、立ち上げ時間は変わる', '収率低下で、反応ラインが止まった', '田中さんが不在。原料ロットに合う反応温度は？', '原料投入速度の上限を、標準作業書へ', '別の反応系列にも、改善事例を伝えたい'],
      descriptions: ['離れた原料タンクをまとめる投入順序を考える反応担当者', '純度分析で反応収率の低下を確認する検査担当者', '原料純度の差と反応温度・pHの設定を前に迷う担当者', '原料投入速度の上限を図で標準書に書き込む手元', '反応ラインの担当者どうしが改善図を見て相談する場面'],
      object: '原料投入順序',
      success: '投入順序を変え、立ち上げ時間を短縮',
      failure: '手順を見誤り、収率規格外が発生',
      repeat: '新人の佐藤も、同じ投入順序に気づいた'
    },
    precision: {
      name: '精密機械器具製造業',
      events: ['部品トレー・工具の配置で、段取り時間は変わる', 'アライメントズレで、組立ラインが止まった', '田中さんが不在。室温に合う安定待ち時間は？', '安定待ち時間の基準を、標準作業書へ', '別の組立ラインにも、改善事例を伝えたい'],
      descriptions: ['離れた部品トレー・工具をまとめる配置を考える組立担当者', '精度測定で光学系のアライメントズレを確認する検査担当者', '室温の変化と締結後の安定待ち時間を前に迷う担当者', '安定待ち時間の基準を図で標準書に書き込む手元', '組立ラインの担当者どうしが改善図を見て相談する場面'],
      object: '部品トレー・工具配置',
      success: '配置を変え、段取り時間を短縮',
      failure: '手順を見誤り、アライメントズレが発生',
      repeat: '新人の佐藤も、同じ配置に気づいた'
    }
  };
